...
3. 理解三个核心抽象：`unit`、`rule`、`repository_mapping`
...
`specFlow` 只有三个正式抽象。其它概念都是从这三个派生出来的。

### 三个抽象

`unit` 是一块可独立描述、实现和验证的工程责任。
它是唯一标准生命周期对象。一个 unit 拥有自己的行为真相、实现计划、实现工作和验证。unit 可以描述一个局部能力，也可以描述一条完整用户结果链。
它不一定等于一个目录、package 或 service。

`rule` 是一个被多个对象正式复用的规则。
它有两种作用域：全局规则（`g_`）作用于整个仓库；绑定规则（`b_`）只作用于通过 `rule_refs` 显式引用它的 unit。
...
`stable` 和 `candidate` 不是独立概念，而是 `unit` 或 `rule` 可以处于的两种层。
...
它记录每个 `unit` 对象当前处于哪一层、下一步合法命令是什么。它不承载行为规则正文。

### 最小模型

```mermaid
flowchart LR
    A["A. unit/rule 处于 stable"] --> B["B. unit/rule 处于 candidate"]
    B --> C["C. 实现和验证"]
    C --> D["D. 升级成新的 stable"]
    D --> A
```
...
1. **维护 spec 文档** —— 编写和更新行为真相文件。unit 的真相在 `docs/specs/units/`。这些是命令消费的真相源头。
