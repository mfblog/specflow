...
[接入仓库](#接入仓库) · [快速开始](#快速开始) · [实际怎么用](#实际怎么用) · [共享真相](#什么时候不再是单元内问题) · [进阶用法](#进阶用法)
...
- `docs/specs/`
  - 这里面会放单元 Spec、appendix 文件、流程状态文件
- `.githooks/pre-commit`
- 其他这套工作流需要的支持文件
...
你真正要写的主文档，就是这个单元 Spec 文件。

一份正式的单元 Spec，至少应该能回答这些问题：
...
如果这个单元还依赖共享真相或全局约束，那么这些对齐关系也要在 Spec 里写明。

下面三个 case，不要把它看成三个零散 FAQ。
更合适的读法是：这是同一个单元从第一次出现，到后续演进，再到后来回头做对齐检查的一条简化生命周期。
...
- `A. 第一版` 是这个单元第一次被正式建立起来
- `B. 下一版` 是同一个单元后面继续演进
- `C. 后续对齐检查` 是过了一段时间后，你回来确认当前代码是不是还符合正式真相

在进入案例前先补一个前提：

- 如果这个单元在 `specFlow` 接入前就已经存在，那么第一次纳管时先用 `unit_init:{unit}`
- 它的作用不是设计下一版，而是先把“当前已经生效的真实行为”落成第一份 governed `stable`
- 从那以后，这个单元就会按下面这条故事线继续走
...
- `unit_new` 先给这个新单元创建第一份 `candidate`
...
- 后面别的 agent 接手时，能直接看到这个单元本来要做什么，而不是只能从代码结果反推
...
- 新单元第一版：`unit_new` + candidate 链
- 已经纳管的单元继续做下一版：`unit_fork` + candidate 链
...
一旦单元已经进入 candidate 链，后面的正常顺序通常是：
...
只有在单元当前停留在 `stable`，而你又想确认代码是不是还和这份 stable 对得上时，才会用到它。
...
`docs/specs/_status.md` 是整个项目的状态索引。
...
```mermaid
flowchart LR
    A["A. 不确定单元当前在哪"] --> B["B. 读 docs/specs/_status.md"]
...
- `C. 看 Active Layer` 是确认这个单元现在处于 `stable` 还是 `candidate`
...
- 让一个单元绑定到已有 shared contract
- 调整 shared 拓扑，比如拆分、合并、重命名、退场
- 改了 shared 之后，检查会影响哪些单元
...
除了标准单元命令，`specFlow` 还有一些更偏治理本身的 flow。
