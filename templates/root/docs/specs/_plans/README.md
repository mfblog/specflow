...
3. `draft/{module}.md` stores non-consumable planning work-in-progress for `unit_plan`.
4. `active/{module}.md` is the only plan file shape that downstream commands may consume.
5. `unit_plan` writes or updates `active/{module}.md` only when the round is `plan-ready`.
6. `unit_plan` may write or update `draft/{module}.md` when planning is blocked, in checkpoint, or still accumulating bounded implementation facts.
7. `unit_impl` and `unit_verify` must consume only `active/{module}.md`.
