...
12. `active/{module}.md` does not carry the gate decision of `unit_check` or the promotion decision of `unit_verify`.
13. `unit_plan` writes or updates `active/{module}.md` only when the round is `plan-ready`.
14. If planning is still blocked or still inside a decision checkpoint, `unit_plan` must keep updating `draft/{module}.md` instead of rewriting `active/{module}.md`.
...
16. For each advanced slice, `unit_impl` should write back at minimum:
   ...
   - `retirement_result`
   - `verification_note`
17. `unit_verify` still requires a current valid `active/{module}.md`.
...
22. `unit_fork` must delete the previous round's `active/{module}.md`.
23. `unit_promote` must delete the corresponding `active/{module}.md`.
