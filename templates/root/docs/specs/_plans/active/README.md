...
8. `Verification Targets` must name which retirement goals `cand_verify` must explicitly prove.
...
12. `active/{module}.md` does not carry the gate decision of `cand_check` or the promotion decision of `cand_verify`.
13. `cand_plan` writes or updates `active/{module}.md` only when the round is `plan-ready`.
14. If planning is still blocked or still inside a decision checkpoint, `cand_plan` must keep updating `draft/{module}.md` instead of rewriting `active/{module}.md`.
15. `cand_impl` may keep writing progress, blockers, verification notes, and retirement progression, but must not rewrite binding fields.
16. For each advanced slice, `cand_impl` should write back at minimum:
   - `execution_surface`
   - `cutover_result`
   - `retirement_result`
   - `verification_note`
17. `cand_verify` still requires a current valid `active/{module}.md`.
18. `cand_verify` must consume `Execution Surface Plan`, `Retirement Targets`, and `Verification Targets` as part of the round's formal verification input.
...
21. Once outdated, the flow must go back to `cand_check` first and then re-run `cand_plan`.
22. `spec_fork` must delete the previous round's `active/{module}.md`.
23. `cand_promote` must delete the corresponding `active/{module}.md`.
