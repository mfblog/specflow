...
2. `surface` 必须引用消费它的命令或内部流程已正式定义的稳定名称，不得由本注册表自行命名。
3. `file` 必须位于 `docs/project_standards/` 下。
4. `consumed_by` 必须显式写出已声明支持该标准类型与 `surface` 的命令名或内部流程名，不得写成 `all`。
...
11. 若某条 entry 引用了未被对应命令或内部流程正式定义的 `surface`，该 entry 不能作为合法治理输入。
