...
2. if the module layer also binds Shared Contract files, it must additionally record `shared_contract_refs` in the same section using the Shared Contract binding contract from `specflow/framework/spec_policy.md` Section 6.1; that field does not replace `system_constraints_ref`
