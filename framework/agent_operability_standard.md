...
### 4.8 Formal Rule Voice

Governance documents must use formal rule voice.

Formal rule voice means the document states the rule, allowed action, forbidden action, owner, dependency, stop condition, or report requirement directly.

The document must not use document-wrapper narration when a direct rule can carry the same meaning.
Document-wrapper narration means self-referential prose that describes the surrounding text instead of stating the rule the executor must follow.

Disallowed patterns include:

1. saying that a block, addendum, section, or document "tells" the reader what it does
2. saying that the surrounding text is "only" an entry guide or similar wrapper label when the needed rule is which downstream policy owns the details
3. introducing a governance object with broad explanation before the first executable rule, when the same meaning can be written as an entry condition or routing rule

Required replacement pattern:

1. name the condition under which the rule applies
2. state the executor action
3. link the owner file for details
4. state the stop or report requirement when relevant
...
11. read document-wrapper narration where a direct rule is required, especially in entry managed blocks or routing documents
