...
6. `specflow/framework/*.md` and `specflow/framework/commands/*.md` are part of repository governance and should normally be committed in the current task.
...
4. when Rule 3 applies, that candidate file must also record the exact `promotion_owner_unit` required by `specflow/framework/spec_policy.md`
5. `MAJOR`
...
### 6.2 `specflow/framework/*.md` And `specflow/framework/commands/*.md`

If the task changes only `specflow/framework/*.md` or `specflow/framework/commands/*.md`:
...
If the task changes a registered entry index file listed in `specflow/framework/entry_index_registry.md`, such as `AGENTS.md`, `GEMINI.md`, or `CLAUDE.md`:
...
3. entry-file sync must be completed before commit; that sync aligns only the managed block defined in `specflow/framework/entry_index_registry.md`
