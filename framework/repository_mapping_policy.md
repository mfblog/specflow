...
`docs/specs/repository_mapping.md` is the durable ownership map for repository paths and formal objects.
...
If a task also changes unit behavior, rule truth, global baseline rules, or unit dependencies, that truth change must be routed through the corresponding policy or command before implementation is claimed complete.
