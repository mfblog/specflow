...
6. direct first-stable onboarding is allowed only when `onboarding_decision_policy.md` proves that the accepted behavior baseline is complete, conflicts are closed, material unknowns are closed or irrelevant, and shared/global truth is resolved
7. if the target only has raw implementation evidence, incomplete evidence, unresolved conflicts, or retained behavior that still needs business confirmation, do not start `unit_init`; route to candidate creation with the required `source_basis` and evidence appendix
8. if onboarding current truth would create duplicated formal truth across units, or if the shared/unit boundary is still unstable, do not start `unit_init`; resolve that shared-truth boundary through natural-language shared governance first
9. if the first `stable` reuses already-existing shared truth, read the relevant `shared_contract` files before writing `shared_contract_refs`
10. if the task also touches global baseline, shared mechanisms, or exceptions, read `docs/specs/system_constraints.md`
11. if the unit involves technical choices, shared infrastructure, cross-unit reuse, global exceptions, or system-level constraint relationships, the first `stable` must include `Global Constraint Alignment` or an equivalent section
12. if the task changes `stable`, `_status.md`, or other commit-triggering governance files, read the git policy first
13. if the round creates, updates, or deletes any unit `shared_contract_refs` value or any file under `docs/specs/shared_contracts/**`, read `specflow/framework/shared_sync.md` first
14. if the round may update `bound_objects` or remove intentional-unbound retention fields from a touched Shared Contract file, read every current-layer unit main file needed to derive the real repository-wide binding set of each touched Shared Contract from `shared_contract_refs`
...
5. create `docs/specs/units/stable/s_unit_{unit}.md`
6. ensure the file covers:
   - `Context & Motivation`
   - `Terminology`
...
7. if needed, add `Global Constraint Alignment` with at least:
   - `system_constraints_ref`
   - `shared_contract_refs` written in the Shared Contract binding contract from `specflow/framework/spec_policy.md` Section 6.1
   - `shared_mechanism_reuse_summary`
   - `global_constraint_exceptions`
8. do not introduce `system_constraints_change_proposal` into the first `stable`; that field belongs only to unit `candidate`
9. if the round changed Shared Contract bindings or touched Shared Contract files:
...
11. if the round changed any unit `shared_contract_refs` value or any file under `docs/specs/shared_contracts/**`, run `shared_sync` after `_status.md` has been updated, even when no additional affected unit is known yet
...
12. perform git close-out if required by policy
...
5. if onboarding evidence was insufficient for first-stable landing, the command stopped before stable writeback and routed to candidate creation with evidence handling
6. the command does not automatically open a candidate round
...
4. whether `Global Constraint Alignment` was required and why
5. whether the command had to stop and reroute through natural-language shared governance because shared-truth boundary closure was required before onboarding could continue
6. whether the command had to stop and route to candidate creation because evidence was not sufficient for direct stable onboarding
