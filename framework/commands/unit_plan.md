...
6. if the pass gate is invalid, stop immediately:
...
7. if the unit already has `stable`, derive a planning-aid change surface before judging the round:
...
8. determine the planning result shape for this round before plan write-back:
...
9. determine whether research preflight is required:
...
10. after research preflight, allow only these three result shapes:
...
11. `decision-checkpoint` is a distinct result shape:
...
12. if the result is `truth-fallback`:
   - delete `_check_result/unit/{unit}.md`
   - delete `_plans/draft/{unit}.md` if it exists
   - delete `_plans/active/{unit}.md` if it exists
   - delete `_verify_result/unit/{unit}.md` if it exists
...
13. if the result is `plan-blocked`:
...
14. determine whether a `decision` checkpoint is required:
...
15. if a `decision` checkpoint is raised:
...
16. create or update `docs/specs/_plans/active/{unit}.md` only when no checkpoint blocks planning and the result is `plan-ready`
17. if `docs/specs/_plans/draft/{unit}.md` exists for the same round and the round is now `plan-ready`, extract only the stabilized planning content into `active/{unit}.md`; do not rename the draft file in place
18. after a successful active-plan write for the current round, delete `docs/specs/_plans/draft/{unit}.md` if it exists
19. identify the changed execution surfaces of this round before finalizing either draft or active planning output:
...
20. for each changed execution surface, record at minimum:
   - current known path
...
21. if current implementation facts are still insufficient to name a target path or retirement goal safely, but candidate truth still stands:
...
22. if planning discovers that the real blocker is missing behavior truth, missing boundary truth, or missing acceptance truth:
   - do not compensate inside plan text
   - treat the round as `truth-fallback`
23. ensure the active plan records:
...
24. complete `Plan Executability` review before treating the round as `plan-ready`:
...
25. treat `plan-ready` as valid only when all of the following hold:
...
27. perform git close-out if required
