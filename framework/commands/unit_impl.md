...
4. run command preflight for `unit_impl:{unit}` and stop before implementation if authoritative validation is unavailable
5. read the current `_check_result/unit/{unit}.md`
6. read the current `_plans/active/{unit}.md`
...
10. only when both pass gate and plan are still valid may implementation continue
11. implement according to the selected intent standard:
   - `change` implements the selected candidate truth
   - `repair` restores the recorded repair basis and must stop if the work requires changing behavior truth, ownership boundary, or acceptance meaning
12. implement slice by slice in the order defined by the current plan unless the plan itself declares a dependency-safe different order
13. for each slice, use the recorded objective, file scope, dependencies, verification action, and done condition as the execution boundary
14. do not collapse a blocked slice into a vague whole-unit status; record clearly which slice is complete, blocked, or still pending
15. treat a slice as truly advanced only when at least one of the following is now true:
...
16. when implementation discovers additional legacy paths, legacy helpers, legacy patches, or legacy wrappers that were not yet fully modeled:
...
17. if implementation discovers that the active plan's convergence target cannot stand without a new behavior or boundary decision:
   - stop treating the issue as implementation-only
   - fall back to `unit_check`
18. run necessary verification for the slices advanced in this round, or record clearly what could not be run
19. write slice completion status, blockers, verification results, and retirement progression back into `_plans/active/{unit}.md`
20. ensure the active plan write-back records at minimum:
   - `Takeover Progress`
   - `Retirement Progress`
...
4. candidate intent execution result
5. takeover-progress result
6. retirement-progress result
7. plan write-back result
8. blocked-slice result when implementation could not finish the current plan round
9. `handoff validation result`
10. cleanup result when implementation stopped through layered recovery
11. `fallback_reason_code` and `failure_layer` when implementation stopped through layered recovery
12. fallback reason when implementation stopped through layered recovery
