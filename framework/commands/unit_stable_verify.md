...
9. classify deviations with the shared severity meanings defined by `specflow/framework/severity_policy.md`
10. conclude:
   ...
   - `partial` and `not_checked` are non-blocking only when `specflow/framework/downgrade_policy.md` allows downgrade for the current evidence state
