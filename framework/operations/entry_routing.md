...
Keyword-matched entries from `framework/governance/review.md` rule 0 (expressions containing "mechanism audit", "governance review", etc.) also resolve to these exact forms.
...
1. **State-table row: "No unit row exists."** Matches when `_status.md` has no entry for the named unit (all `Stable`/`Candidate`/`Active Layer`/`Next Command` columns are absent for the target object). Read the Onboarding Source Decision section of this file. Select `unit_init:{unit}` only when all of the following hold: (a) the `_status.md` table is not empty (at least one unit row exists), (b) the target unit has no row in `_status.md`, (c) the capability's behavior, boundaries, acceptance criteria, and ownership are already fully documented as accepted stable truth outside specFlow — meaning the executor can write a stable Spec from existing repository materials without creating new candidate truth, discovering missing behavior, or resolving ownership during writeback. Select `unit_new:{unit}` when `unit_init` conditions are not met or the request explicitly creates new behavior truth. If neither selection can be proven from current truth and the request, stop and report the missing decision or prerequisite.
...
8. a review entry form does not match any recognized review entry (exact or keyword-matched per `framework/governance/review.md` rule 0) — stop per `framework/governance/review.md` "Unrecognized Entry"
