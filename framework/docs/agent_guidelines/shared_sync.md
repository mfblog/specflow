...
6. read the current in-scope `shared_contract` files under:
   - `docs/specs/shared_contracts/stable/`
   - `docs/specs/shared_contracts/candidate/`
7. if the current task changed any current-layer truth file or binding source used to derive downstream scope, that writeback must already be present before `shared_sync` computes impact
...
2. rebuild the real binding set from downstream formal truth:
   - `unit.shared_contract_refs`
   - `scenario.shared_contract_refs`
...
3. apply the dependency direction from `spec_policy.md`:
   - `shared_contract -> unit/scenario`
4. if a shared change also changes the repository object map or path-ownership truth:
   - update `docs/specs/repository_mapping.md` before handing off to `impact_sync`
5. if repository truth is insufficient to determine the real binding set safely:
   - stop `shared_sync`
...
2. the affected downstream object set grouped by:
   - `unit`
   - `scenario`
3. whether `docs/specs/repository_mapping.md` had to be updated before impact handoff
...
7. whether control was returned to `shared_escape`
