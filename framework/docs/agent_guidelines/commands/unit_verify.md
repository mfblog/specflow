...
9. verify the changed execution surfaces named by the active plan, not just the module as an undifferentiated whole
10. produce both:
   - the structured verification evidence matrix
   - a `Structure Convergence Matrix`
11. for each changed execution surface in the `Structure Convergence Matrix`, report at minimum:
   - `execution_surface`
   - `behavior_alignment`
   ...
   - `retirement_result`
   - `deviation_reason`
12. treat any of the following as direct `implementation_deviation`:
...
5. if pass gate or plan was invalid, verification stopped and `_status.md` fell back to `unit_check`
...
8. cleanup result when verification fell back to `unit_check`
...
11. deviation list
12. fallback reason if pass gate or plan was invalid
