...
4. when the round retargets a module away from one shared file to another, update the previous shared file's declarative `bound_objects` metadata too
...
10. if the round may update `bound_objects`, validate or rewrite `promotion_owner_module`, or resolve the terminal state of a touched Shared Contract file, resolve the repository-wide real binding set of each touched Shared Contract from current repository truth before writeback:
...
12. if the target module is currently at `stable`, also read `specflow/framework/docs/agent_guidelines/commands/unit_fork.md`
13. read `specflow/framework/docs/agent_guidelines/git_policy.md` when the round may change module `shared_contract_refs`, update `bound_objects`, validate or rewrite `promotion_owner_module`, delete a touched Shared Contract file, or otherwise mutate commit-triggering governance files
...
2. if the target module current layer is `stable`, do not modify `stable` directly:
   ...
   - require `unit_fork:{module}` to create the target module candidate first
...
4. resolve the repository-wide real binding set of the target shared file and any previous bound Shared Contract file from current repository truth before shared metadata writeback:
   - derive that set from current-layer module `shared_contract_refs` rather than from `bound_objects`
...
8. update the target shared file's `bound_objects` only as declarative metadata so it matches the real binding set implied by the repository-wide module-side `shared_contract_refs` plus this round's prepared target-module writeback
9. if Step 3 recorded a previous bound Shared Contract file and it is different from the new target file, update that previous shared file's `bound_objects` only after reconciling it against the same repository-wide binding view plus this round's prepared target-module writeback
...
1. the module binding, body-level consumption explanation, and target shared file `bound_objects` metadata are complete and `shared_sync` has finished reconciliation
...
4. the target module is currently at `stable` and the flow has raised a `shared_ops` checkpoint for `unit_fork:{module}` first
...
3. whether the target module was already at `candidate` or first had to stop for `unit_fork:{module}`
...
8. when the round retargeted the module away from a previous shared file, the previous shared file `bound_objects` reconciliation result
