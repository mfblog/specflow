...
6. a `shared_ops` or internal shared-governance flow must determine affected downstream objects from current repository structure
...
3. compare the classification with the command target
   ...
   - a `shared_ops` flow may operate on declared shared-contract truth and binding metadata, but must not silently rewrite unit behavior truth
