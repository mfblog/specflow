...
5. project-side process and state contracts
...
6. project repository-structure truth
...
10. tooling contract and tooling source input
...
3. the impact-reconciliation rule set
   - at minimum `impact_sync_policy.md`, `process_snapshot_contract.md`, `recovery_policy.md`, template `_status.md`, project `_status.md`, and the template-side and project-side process README files
...
The current `docs/specs/repository_mapping.md` truth and repository-mapping policy are reviewed inside `review_and_command_core`, plus their process and tooling contracts in the other fixed blocks.
...
3. review each fixed block for:
   ...
   - contract drift
   - missing ownership
4. review the `agent_operability` block against `specflow/framework/agent_operability_standard.md`
...
6. the guidance-skill coverage result
7. the impact-reconciliation coverage result
8. the tooling coverage result
9. the agent-operability result, including execution clarity, content economy, and formal rule voice
10. the cross-block convergence results
11. the findings result:
   - explicit `none` when no real finding exists
   - otherwise every finding must satisfy Section 7.1
12. the final conclusion:
   - `pass`
   - `blocked`

If the output does not explicitly report Items 5 through 11, the review is not complete.
