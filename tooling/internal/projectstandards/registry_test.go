...
func TestValidateRegistryPassesForSpecFlowResponseOutputStandard(t *testing.T) {
	repoRoot := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repoRoot, "docs/project_standards"), 0o755); err != nil {
		t.Fatalf("mkdir project standards: %v", err)
	}
	...
	registry := strings.Join([]string{
		...
		"| standard_id | type | surface | file | consumed_by | applies_to | effect | conflict_rule | notes |",
		"|---|---|---|---|---|---|---|---|---|---|",
		"| `response_clarity` | `output_standard` | `user_facing_response_clarity` | `docs/project_standards/specflow_response_clarity.md` | `specflow_response` | `all_targets_on_surface` | `tighten` | `framework_wins` | note |",
	}, "\n") + "\n"
	if err := os.WriteFile(filepath.Join(repoRoot, "docs/project_standards/_registry.md"), []byte(registry), 0o644); err != nil {
		t.Fatalf("write registry: %v", err)
	}
	if err := os.WriteFile(filepath.Join(repoRoot, "docs/project_standards/specflow_response_clarity.md"), []byte("# Response"), 0o644); err != nil {
		t.Fatalf("write standard file: %v", err)
	}
