...
func TestDetermineTransitionCoversStandardOutcomes(t *testing.T) {
	cases := []struct {
		...
		{name: "unit_stable_verify truth_text_change_required", opts: Options{Command: "unit_stable_verify", ObjectType: "unit", Object: "demo", Outcome: "truth_text_change_required"}, current: unitStableStatus("unit_stable_verify"), present: true, wantStable: "yes", wantCandidate: "no", wantActiveLayer: "stable", wantNext: "unit_fork", wantValidation: "stable_verify", wantCleanupKind: cleanupNone},
		...
		{name: "unit_check pass", opts: Options{Command: "unit_check", ObjectType: "unit", Object: "demo", Outcome: "pass"}, current: unitCandidateStatus("unit_check"), present: true, wantStable: "no", wantCandidate: "yes", wantActiveLayer: "candidate", wantNext: "unit_check, unit_impl, unit_verify", wantValidation: "check", wantCleanupKind: cleanupNone},
...
func TestCloseUnitCheckPassUsesCurrentEvidenceDespiteOldCheckWork(t *testing.T) {
	for _, checkWorkStatus := range []string{"blocked_on_finding", "closed_fix_required"} {
		t.Run(checkWorkStatus, func(t *testing.T) {
			...
			if status.NextCommand != "unit_check, unit_impl, unit_verify" {
				t.Fatalf("current valid check evidence must advance to implementation phase, got NextCommand=%q", status.NextCommand)
			}
		})
	}
}
...
func TestCloseUnitVerifyTruthFallbackFallbackCleanupFailureReportsRetryAfterStatusUpdate(t *testing.T) {
	repoRoot := commandCloseSnapshotRepo(t, "| `unit` | `demo` | `no` | `yes` | `candidate` | `unit_verify` | test |\n")
	writeCommandCloseStableSpec(t, repoRoot)
	snap, err := snapshot.RebuildCurrentObject(repoRoot, "unit", "demo")
	if err != nil {
	...
	// Replace the verify result file with a directory to block fallback cleanup
	verifyPath := filepath.Join(repoRoot, "docs/specs/_verify_result/unit/demo.md")
	if err := os.Remove(verifyPath); err != nil {
		t.Fatalf("remove verify result: %v", err)
	}
	blockingDir := verifyPath
	if err := os.MkdirAll(blockingDir, 0o755); err != nil {
		t.Fatalf("mkdir blocking verify dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(blockingDir, "child.txt"), []byte("blocked"), 0o644); err != nil {
	...
	if err == nil || !strings.Contains(err.Error(), "fallback cleanup failed after status update") || !strings.Contains(err.Error(), "process cleanup-fallback") {
		t.Fatalf("expected retryable fallback cleanup failure after status update with process cleanup-fallback, got %v", err)
	}
	if !result.StatusUpdated {
		t.Fatalf("expected status update before fallback cleanup failure")
	}
	// Verify status reflects the expected fallback transition
	status, err := statusfile.LookupObjectStatus(repoRoot, "unit", "demo")
	if err != nil {
		t.Fatalf("LookupObjectStatus: %v", err)
	}
	if status.NextCommand != "unit_check" {
		t.Fatalf("status must reflect truth_fallback transition, got NextCommand=%s", status.NextCommand)
	}
}
