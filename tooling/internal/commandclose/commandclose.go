...
func Close(opts Options) (Result, error) {
	...
	case cleanupFallback:
		// Write status first, then clean up fallback files.
		// If cleanup fails after the status is written, the error includes a retry
		// instruction to re-run the fallback cleanup. This avoids the case where
		// files are deleted but status is not updated (original ordering).
		...
		if err != nil {
			if result.StatusUpdated {
				return result, fallbackCleanupAfterStatusError(opts, err)
			}
			return result, err
		}
...
func fallbackCleanupAfterStatusError(opts Options, err error) error {
	return fmt.Errorf(
		"fallback cleanup failed after status update; fix the filesystem blocker and rerun `specflowctl process cleanup-fallback --repo-root %s --object-type %s --object %s --from-command %s --reason %s --failure-layer %s`: %w",
		opts.RepoRoot, opts.ObjectType, opts.Object, opts.Command, opts.Reason, opts.FailureLayer, err)
}
...
func unitStableVerifyTransition(opts Options, current statusfile.ObjectStatus) transition {
	switch opts.Outcome {
	case "aligned", "truth_text_change_required":
