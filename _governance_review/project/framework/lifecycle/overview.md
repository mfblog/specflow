...
**Design principle.** Every context card must inline execution authority (WRITES, READS, BLOCKED),
lifecycle boundary (STATUS), and close guidance (GUIDANCE — pre-checks through outcome routing and
close syntax). It may index business truth files in References as read-only paths; only `_status.md`
and `repository_mapping.md` are inlined in Core Truth. This principle governs both card generation
and card review.
...
> **Terminology note:** SpecFlow has two card types. (1) **Per-State Card** — generated by `specflowctl context card`, the primary execution guide with state-specific STATUS, GUIDANCE, and Core Truth inlined. (2) **Per-Command Card** — the lifecycle file at `framework/lifecycle/unit_*.md`, used when the generated card's GUIDANCE is insufficient. When any file uses "Context Card" without qualification, it refers to the Per-State (generated) card. "Lifecycle Context Card" or "Per-Command Card" refers explicitly to the lifecycle file.
