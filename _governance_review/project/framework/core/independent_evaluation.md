...
### Tooling-Unavailable Fallback

When `specflowctl evaluation request` is unavailable:

1. Create the request file manually at the standard path: `docs/specs/_independent_evaluation/requests/unit/{unit}/{reviewer_pack}.md`
2. Include the following required fields in the request file:
   - `reviewer_pack`: the pack name from the active Context Card
   - `review_standard_refs`: authoritative criteria refs (see the reviewer pack definition below)
   - `review_file_refs`: durable truth refs the reviewer needs to read
   - `review_evidence_refs`: process evidence refs for the reviewer to inspect
   - `durable_input_refs`: the refs that will be recorded in the receipt's `review_input_refs`
3. Proceed with the handoff procedure below as if the request file was generated by tooling.
4. The `review_input_refs` in the process evidence must record `manual_request` in addition to the reviewer pack name to indicate tooling was bypassed.

Example minimal request file:

```yaml
reviewer_pack: unit_check_pass
review_standard_refs:
...
durable_input_refs: unit_check_pass;docs/specs/_independent_evaluation/requests/unit/{unit}/unit_check_pass.md;docs/specs/units/candidate/c_unit_{unit}.md
```

See `tooling/internal/evaluationrequest/evaluationrequest.go` for the authoritative request file structure.
...
Review standard refs list the source documents for the evaluation criteria; the Evaluation Questions in the reviewer pack are the authoritative criteria for the reviewer decision.
...
### In-Session Subagent Isolation

`reviewer_context: minimal_context` requires that the reviewer session does not
inherit the executor's full working context.

When the agent runtime evaluates within the same session — for example, an
in-session subagent spawned inside the executor's conversation — the subagent
inherits the executor's full conversation history by default. This inherited
context may include:

1. executor chain-of-thought, draft rationale, or private notes.
2. broad repository scan results not named by the review pack.
3. unverified assumptions from the implementation session.
4. unrelated policy, governance, recovery, or migration context.
5. prior chat conclusions not recorded as durable human-confirmation refs.

**Requirements for valid in-session subagent review:**

1. The subagent must be created with a restricted context that includes only
   the content explicitly authorized by the review pack and the request file.
2. The inherited session context must not be used as authority for the review
   decision.
3. The executor must not rely on shared conversation state to influence the
   reviewer.
4. The `review_input_refs` field must accurately list only the durable refs
   supplied to the reviewer, not the full session scope.

**Tooling limitations:**

The tooling validates the receipt fields mechanically (see
`process_snapshot_contract.md` Section 2). It does not:

1. prove that the reviewer session was actually isolated.
2. detect whether inherited context influenced the review decision.
3. enforce subagent context restrictions.

For a fully isolated review that satisfies `minimal_context` without ambiguity,
the reviewer must run in a separate agent session that has no access to the
executor's conversation history.

Process evidence recording `reviewer_context: minimal_context` while using an
in-session subagent with unrestricted inherited context is not valid
...
**Scope Verification:**
5. For acceptance items that declare `affects`, does the `scope_verification` record confirm that all affected files, appendices, rules, and dependencies were verified?

**Code Quality:**
6. Does the implementation contain dead code, unnecessary abstractions, or duplicated logic that could be simplified?
7. Is the implementation concise and proportional to the acceptance item scope? (For replacement scenes: is the new code volume proportionate to the replaced code volume?)
8. Does the implementation introduce over-engineering (layers, interfaces, or patterns not justified by current requirements)?

**Retirement Completeness (replacement scenes only):**
9. Are the old code paths declared in `affects.files` fully removed (not left as dead wrappers, compatibility stubs, or commented-out code)?
10. Is there any remaining module, test, or configuration that references the deleted paths?
...
4. If the decision is `truth_text_change_required`, does the evidence prove that the stable truth text must change and cannot be resolved through re-interpretation of the existing text?
