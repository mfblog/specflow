...
func ReleaseVersion(repoRoot string, options ReleaseVersionOptions) (ReleaseVersionResult, error) {
	...
	for _, status := range statuses {
		...
		if status.NextCommand != "unit_fork" { nonCompliant = append(nonCompliant, fmt.Sprintf("%s %q", status.ObjectType, status.Object)); continue }
	...
	if len(nonCompliant) > 0 {
		return ReleaseVersionResult{}, fmt.Errorf("rule release-version stable auto-fork requires all stable consumers of old ref %q to be at Next Command: unit_fork. Non-compliant units: %s", normalized.FromRef, strings.Join(nonCompliant, ", "))
	}
