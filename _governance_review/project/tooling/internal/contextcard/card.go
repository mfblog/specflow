...
func writeUnitGuidance(buf *strings.Builder, repoRoot, unitName, implPaths string, state UnitState, adoptionMode string) string {
	...
	rawText = strings.ReplaceAll(rawText, "<unit>", unitName)
	rawText = strings.ReplaceAll(rawText, "{outcome}", "the_outcome_for_your_command")
	rawText = strings.ReplaceAll(rawText, "<outcome>", "the_outcome_for_your_command")
...
func writeUnitWrites(buf *strings.Builder, state UnitState, unitName, implPaths string) {
	...
	case StateUnregistered:
		buf.WriteString("- See GUIDANCE \"How to End\" table for per-command write targets\n")
	}
	buf.WriteString("\n")
}
...
func writeUnitClose(buf *strings.Builder, state UnitState, unitName string, lifecycleContent string) {
	...
	if closeCmd != "" {
		...
		closeCmd = strings.ReplaceAll(closeCmd, "{unit}", unitName)
		closeCmd = strings.ReplaceAll(closeCmd, "<outcome>", "the_outcome")
		closeCmd = strings.ReplaceAll(closeCmd, "{outcome}", "the_outcome")
	...
	if state == StateCandidatePending {
		...
		fmt.Fprintf(buf, "If spec issues are found during implementation, fix the candidate spec first, then run unit_check:%[1]s for re-validation.\n\n", unitName)
		return
	}
...
func findAffectedUnits(repoRoot, ruleID, scope string) []AffectedUnitRow {
	...
	for _, st := range statuses {
		...
		for _, ref := range refs {
			...
			if prefix == "s_"+ruleID || prefix == "c_"+ruleID || prefix == ruleID {
				matched = true
				break
			}
		}
...
func parseNamedRefs(content, field string) []string {
	...
	for idx, line := range lines {
		...
		if right != "" {
			// Handle inline list form: `rule_refs: - s_rule@1.0.0 - c_rule@0.1.0`
			// Parse as space-separated tokens, stripping "- " prefix from each.
