...
func TestValidateProcessFileRejectsNonPassCheckAndVerifyDecisions(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.processKind+"/"+tc.decision, func(t *testing.T) {
			...
			if tc.processKind == "verify" {
				activePlanFingerprint := writeFormalPlanProcessForTest(t, repoRoot, expected)
				body = renderFormalVerifyProcessBody(expected, activePlanFingerprint)
			}
...
func TestValidateProcessFileAllowsPlanWithoutReceipt(t *testing.T) {
	repoRoot := t.TempDir()
	setupSnapshotValidationRepo(t, repoRoot)
	...
	body := strings.Replace(renderFormalPlanProcessBody(expected), renderIndependentEvaluationReceiptForTest(expected.Object, "plan_ready", expected.SpecFileRef), "", 1)
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_plans/active/demo.md"), "# plan\n\n```yaml\n"+body+"\n```\n")
	...
	if !result.Valid {
		t.Fatalf("expected plan without receipt to pass standard validation, got mismatches %+v", result.Mismatches)
	}
...
func TestValidateProcessFileRejectsPassWithReviewFindings(t *testing.T) {
	...
	activePlanFingerprint := writeFormalPlanProcessForTest(t, repoRoot, expected)
	body := strings.Replace(renderFormalVerifyProcessBody(expected, activePlanFingerprint), "review_findings: none", "review_findings: missing-acceptance-boundary", 1)
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit/demo.md"), "# verify\n\n```yaml\n"+body+"\n```\n")
...
func TestValidateProcessFileForIndependentEvaluationRequestRejectsCoverageGap(t *testing.T) {
	...
	body = strings.Replace(body, renderIndependentEvaluationReceiptForTest(expected.Object, "plan_ready", expected.SpecFileRef), "", 1)
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_plans/active/demo.md"), "# plan\n\n```yaml\n"+body+"\n```\n")
...
func TestValidateProcessFileRejectsPlanMissingRetirementTargets(t *testing.T) {
	repoRoot := t.TempDir()
	setupSnapshotValidationRepo(t, repoRoot)
	...
	body := strings.Replace(renderFormalPlanProcessBody(expected), "retirement_targets: none\n", "", 1)
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_plans/active/demo.md"), "# plan\n\n```yaml\n"+body+"\n```\n")
...
func TestValidateProcessFileRejectsPlanRetirementTargetGaps(t *testing.T) {
	for _, tc := range []struct {
		name string
		body string
		want string
	}{
		{
			...
			body: strings.Join([]string{
				...
				"  - id: rt.legacy",
				"    target_kind: helper",
				"    retirement_method: reroute",
				"    verification_action: go test ./...",
				"    acceptance_item_ids: demo.core",
			}, "\n"),
			want: "retirement_targets invalid: each item must include id, target_ref, target_kind, retirement_method, verification_action, and acceptance_item_ids",
		},
		{
			name: "duplicate id",
			body: strings.Join([]string{
				"retirement_targets:",
				"  - id: rt.legacy",
				"    target_ref: internal/legacy.go",
				"    target_kind: helper",
				"    retirement_method: reroute",
				"    verification_action: go test ./...",
				"    acceptance_item_ids: demo.core",
				"  - id: rt.legacy",
				"    target_ref: internal/legacy_wrapper.go",
				"    target_kind: wrapper",
				"    retirement_method: replace",
				"    verification_action: go test ./...",
				"    acceptance_item_ids: demo.core",
			}, "\n"),
			want: "retirement_targets duplicate id: rt.legacy",
		},
		{
			name: "unknown acceptance item",
			body: strings.Join([]string{
				"retirement_targets:",
				"  - id: rt.legacy",
				"    target_ref: internal/legacy.go",
				...
				"    retirement_method: reroute",
				"    verification_action: go test ./...",
				"    acceptance_item_ids: demo.other",
			}, "\n"),
			want: "retirement_targets unknown acceptance_item_ids for rt.legacy: demo.other",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			repoRoot := t.TempDir()
			...
			body := strings.Replace(renderFormalPlanProcessBody(expected), "retirement_targets: none", tc.body, 1)
			mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_plans/active/demo.md"), "# plan\n\n```yaml\n"+body+"\n```\n")
			...
			if !containsMismatch(result.Mismatches, tc.want) {
				t.Fatalf("expected mismatch %q, got %+v", tc.want, result.Mismatches)
			}
		})
	}
}
...
func TestValidateProcessFileRejectsReceiptWithoutRequestRefs(t *testing.T) {
	...
	activePlanFingerprint := writeFormalPlanProcessForTest(t, repoRoot, expected)
	body := strings.Replace(renderFormalVerifyProcessBody(expected, activePlanFingerprint), reviewInputRefsForTest(expected.Object, "unit_verify_ready_to_promote", expected.SpecFileRef), expected.SpecFileRef, 1)
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit/demo.md"), "# verify\n\n```yaml\n"+body+"\n```\n")
	...
	for _, want := range []string{
		"review_input_refs must contain reviewer pack: unit_verify_ready_to_promote",
		"review_input_refs must contain request file ref: docs/specs/_independent_evaluation/requests/unit/demo/unit_verify_ready_to_promote.md",
	} {
		if !containsMismatch(result.Mismatches, want) {
			t.Fatalf("expected mismatch %q, got %+v", want, result.Mismatches)
		}
...
func TestValidateProcessFileRejectsVerifyEvidenceGap(t *testing.T) {
	...
	activePlanFingerprint := writeFormalPlanProcessForTest(t, repoRoot, expected)
	body := strings.Replace(renderFormalVerifyProcessBody(expected, activePlanFingerprint), "status: pass", "status: skipped", 1)
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit/demo.md"), "# verify\n\n```yaml\n"+body+"\n```\n")
...
func TestValidateProcessFileAcceptsVerifyWithoutActivePlanRef(t *testing.T) {
	repoRoot := t.TempDir()
	setupSnapshotValidationRepo(t, repoRoot)
	mustMkdirAll(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit"))
	...
	body := renderNoPlanVerifyProcessBody(expected)
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit/demo.md"), "# verify\n\n```yaml\n"+body+"\n```\n")
	...
	if !result.Valid {
		t.Fatalf("expected valid result (active_plan fields are optional), got mismatches %+v", result.Mismatches)
	}
}
...
func TestValidateProcessFileRejectsVerifyMissingPackageDeltaVerification(t *testing.T) {
	...
	if !containsMismatchPrefix(result.Mismatches, "package_delta_verification missing planned_change_scope_id") {
		t.Fatalf("expected package_delta_verification mismatch, got %+v", result.Mismatches)
	}
}
...
func TestValidateProcessFileRejectsVerifyActivePlanFingerprintDrift(t *testing.T) {
	repoRoot := t.TempDir()
	setupSnapshotValidationRepo(t, repoRoot)
	mustMkdirAll(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit"))
	...
	writeFormalPlanProcessForTest(t, repoRoot, expected)
	body := renderFormalVerifyProcessBody(expected, "stale-plan-fingerprint")
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit/demo.md"), "# verify\n\n```yaml\n"+body+"\n```\n")
	...
	if !containsMismatchPrefix(result.Mismatches, "active_plan_fingerprint mismatch: actual=stale-plan-fingerprint") {
		t.Fatalf("expected active plan fingerprint mismatch, got %+v", result.Mismatches)
	}
}

func TestValidateProcessFileRequiresRetirementEvidenceForPlanTargets(t *testing.T) {
	repoRoot := t.TempDir()
	setupSnapshotValidationRepo(t, repoRoot)
	mustMkdirAll(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit"))
	...
	activePlanFingerprint := writeFormalPlanProcessForTestWithRetirementTargets(t, repoRoot, expected)
	body := renderFormalVerifyProcessBody(expected, activePlanFingerprint)
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit/demo.md"), "# verify\n\n```yaml\n"+body+"\n```\n")
	...
	if !containsMismatch(result.Mismatches, "retirement_evidence_matrix missing id: rt.legacy") {
		t.Fatalf("expected missing retirement evidence mismatch, got %+v", result.Mismatches)
	}
}

func TestValidateProcessFileRejectsIncompleteRetirementEvidence(t *testing.T) {
	for _, tc := range []struct {
		name   string
		result string
		dep    string
		...
		{name: "fail", result: "fail", dep: "not_required", want: "retirement_evidence_matrix result for rt.legacy must be pass"},
		{name: "not checked", result: "not_checked", dep: "not_required", want: "retirement_evidence_matrix result for rt.legacy must be pass"},
		{name: "still required", result: "pass", dep: "still_required", want: "retirement_evidence_matrix mainline_dependency for rt.legacy must be not_required"},
		{name: "unknown dependency", result: "pass", dep: "unknown", want: "retirement_evidence_matrix mainline_dependency for rt.legacy must be not_required"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			repoRoot := t.TempDir()
			setupSnapshotValidationRepo(t, repoRoot)
			...
			activePlanFingerprint := writeFormalPlanProcessForTestWithRetirementTargets(t, repoRoot, expected)
			body := strings.Replace(renderFormalVerifyProcessBody(expected, activePlanFingerprint), "retirement_evidence_matrix: none", renderRetirementEvidenceForTest(tc.result, tc.dep), 1)
			mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit/demo.md"), "# verify\n\n```yaml\n"+body+"\n```\n")
...
func TestValidateProcessFileAcceptsCompleteRetirementEvidence(t *testing.T) {
	repoRoot := t.TempDir()
	setupSnapshotValidationRepo(t, repoRoot)
	mustMkdirAll(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit"))
	...
	activePlanFingerprint := writeFormalPlanProcessForTestWithRetirementTargets(t, repoRoot, expected)
	body := strings.Replace(renderFormalVerifyProcessBody(expected, activePlanFingerprint), "retirement_evidence_matrix: none", renderRetirementEvidenceForTest("pass", "not_required"), 1)
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_verify_result/unit/demo.md"), "# verify\n\n```yaml\n"+body+"\n```\n")
...
func renderFormalPlanProcessBody(expected Snapshot) string {
	return strings.Join([]string{
		...
		"package_constraint_refs: " + expected.SpecFileRef,
		"package_constraint_summary: current package constraints reviewed for this delta",
		renderIndependentEvaluationReceiptForTest(expected.Object, "plan_ready", expected.SpecFileRef),
	}, "\n")
}

func renderFormalVerifyProcessBody(expected Snapshot, activePlanFingerprint string) string {
	return strings.Join([]string{
		"object_type: unit",
		"object_ref: " + expected.Module,
		"gate: unit_verify",
...
func renderNoPlanVerifyProcessBody(expected Snapshot) string {
	return strings.Join([]string{
		"object_type: unit",
		"object_ref: " + expected.Module,
		"gate: unit_verify",
...
func writeFormalPlanProcessForTest(t *testing.T, repoRoot string, expected Snapshot) string {
	t.Helper()
	return writeFormalPlanProcessBodyForTest(t, repoRoot, renderFormalPlanProcessBody(expected))
}

func writeFormalPlanProcessForTestWithRetirementTargets(t *testing.T, repoRoot string, expected Snapshot) string {
	t.Helper()
	body := strings.Replace(renderFormalPlanProcessBody(expected), "retirement_targets: none", renderRetirementTargetsForTest(), 1)
	return writeFormalPlanProcessBodyForTest(t, repoRoot, body)
}

func writeFormalPlanProcessBodyForTest(t *testing.T, repoRoot, body string) string {
	t.Helper()
	content := "# plan\n\n```yaml\n" + body + "\n```\n"
	mustWriteFile(t, filepath.Join(repoRoot, "docs/specs/_plans/active/demo.md"), content)
	return hashNormalizedText(content)
}

func renderRetirementTargetsForTest() string {
	return strings.Join([]string{
		"retirement_targets:",
		"  - id: rt.legacy",
		"    target_ref: internal/legacy.go",
...
func renderRetirementEvidenceForTest(result, dependency string) string {
	return strings.Join([]string{
		"retirement_evidence_matrix:",
		"  - id: rt.legacy",
		"    result: " + result,
		"    mainline_dependency: " + dependency,
		"    evidence_refs: go test ./...",
	}, "\n")
}
...
func containsMismatchPrefix(mismatches []string, prefix string) bool {
	for _, mismatch := range mismatches {
...
func TestComputeAcceptanceBehaviorFingerprintIsDeterministic(t *testing.T) {
	entries := []AcceptanceItemEntry{
		{ID: "b.item", Target: "B target", VerificationSurface: "integration", ImplementationSurface: "svc/b", VerificationMethod: "integration test", PassCondition: "passes", NotRunnableYet: "no"},
		{ID: "a.item", Target: "A target", VerificationSurface: "public_api", ImplementationSurface: "svc/a", VerificationMethod: "unit test", PassCondition: "passes", NotRunnableYet: "yes", NotRunnableYetReason: "waiting on API"},
	}
	fp1 := ComputeAcceptanceBehaviorFingerprint(entries)
	fp2 := ComputeAcceptanceBehaviorFingerprint(entries)
	if fp1 != fp2 {
		t.Fatalf("fingerprint is not deterministic: first=%s second=%s", fp1, fp2)
	}
	if fp1 == "" {
...
func TestComputeAcceptanceBehaviorFingerprintSortOrder(t *testing.T) {
	reversed := []AcceptanceItemEntry{
		{ID: "z.last", Target: "Z", VerificationSurface: "manual_effect", ImplementationSurface: "svc/z", VerificationMethod: "manual", PassCondition: "checked", NotRunnableYet: "no"},
		{ID: "a.first", Target: "A", VerificationSurface: "internal_flow", ImplementationSurface: "svc/a", VerificationMethod: "test", PassCondition: "passes", NotRunnableYet: "no"},
	}
	sorted := []AcceptanceItemEntry{
		{ID: "a.first", Target: "A", VerificationSurface: "internal_flow", ImplementationSurface: "svc/a", VerificationMethod: "test", PassCondition: "passes", NotRunnableYet: "no"},
		{ID: "z.last", Target: "Z", VerificationSurface: "manual_effect", ImplementationSurface: "svc/z", VerificationMethod: "manual", PassCondition: "checked", NotRunnableYet: "no"},
	}
	fpReversed := ComputeAcceptanceBehaviorFingerprint(reversed)
	fpSorted := ComputeAcceptanceBehaviorFingerprint(sorted)
	if fpReversed != fpSorted {
		t.Fatalf("fingerprint depends on input order: reversed=%s sorted=%s", fpReversed, fpSorted)
	}
}

func TestComputeAcceptanceBehaviorFingerprintEmptyFields(t *testing.T) {
	withTarget := []AcceptanceItemEntry{
		{ID: "a", Target: "defined", VerificationSurface: "public_api", ImplementationSurface: "svc/a", VerificationMethod: "test", PassCondition: "ok", NotRunnableYet: "no"},
	}
	withoutTarget := []AcceptanceItemEntry{
		{ID: "a", Target: "", VerificationSurface: "public_api", ImplementationSurface: "svc/a", VerificationMethod: "test", PassCondition: "ok", NotRunnableYet: "no"},
	}
	fp1 := ComputeAcceptanceBehaviorFingerprint(withTarget)
	fp2 := ComputeAcceptanceBehaviorFingerprint(withoutTarget)
	if fp1 == fp2 {
		t.Fatalf("fingerprint must differ when target changes between empty and non-empty")
	}
}
