...
func TestSnapshotComputeTruthFingerprintCLI(t *testing.T) {
	repoRoot := createCLISnapshotRepo(t)
	expected, err := snapshot.RebuildCurrentObject(repoRoot, "unit", "demo")
	if err != nil {
		t.Fatalf("RebuildCurrentObject: %v", err)
	...
	if err != nil {
		t.Fatalf("compute-fingerprint failed: %v\nstderr=%s", err, stderr.String())
	}
	output := stdout.String()
	if !strings.Contains(output, "truth_fingerprint: "+expected.SpecFingerprint) {
		t.Fatalf("expected truth_fingerprint %s in output, got %s", expected.SpecFingerprint, output)
	}
	if !strings.Contains(output, "algorithm: sha256 of normalized spec file text") {
		t.Fatalf("expected algorithm description in output, got %s", output)
	}
}

func TestSnapshotComputeAcceptanceBehaviorFingerprintCLI(t *testing.T) {
	repoRoot := createCLISnapshotRepo(t)
	expected, err := snapshot.RebuildCurrentObject(repoRoot, "unit", "demo")
	if err != nil {
		t.Fatalf("RebuildCurrentObject: %v", err)
	...
	err = runSnapshot([]string{
		...
		"--object-type", "unit",
		"--object", "demo",
		"--field", "acceptance_behavior_fingerprint",
		"--repo-root", repoRoot,
	}, &stdout, &stderr)
	if err != nil {
		t.Fatalf("compute-fingerprint failed: %v\nstderr=%s", err, stderr.String())
	...
	output := stdout.String()
	if !strings.Contains(output, "acceptance_behavior_fingerprint: "+expected.AcceptanceBehaviorFingerprint) {
		t.Fatalf("expected acceptance_behavior_fingerprint %s in output, got %s", expected.AcceptanceBehaviorFingerprint, output)
	}
	if !strings.Contains(output, "serialization: key=value") {
		t.Fatalf("expected serialization description in output, got %s", output)
	}
}

func TestSnapshotComputeFileFingerprintCLI(t *testing.T) {
	repoRoot := createCLISnapshotRepo(t)
	tmpFile := filepath.Join(repoRoot, "test_content.md")
	content := "hello world\n"
	writeCLITestFile(t, tmpFile, content)
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	err := runSnapshot([]string{
		"compute-fingerprint",
	...
	output := stdout.String()
	expectedFP := snapshot.ComputeFileFingerprint(content)
	if !strings.Contains(output, "file_fingerprint: "+expectedFP) {
		t.Fatalf("expected file_fingerprint %s in output, got %s", expectedFP, output)
	}
	if !strings.Contains(output, "file: "+filepath.ToSlash(tmpFile)) {
		t.Fatalf("expected file path in output, got %s", output)
	}
}

func TestSnapshotComputeFingerprintRejectsMissingField(t *testing.T) {
	repoRoot := createCLISnapshotRepo(t)
	var stderr bytes.Buffer
	var stdout bytes.Buffer
	err := runSnapshot([]string{
	...
	if err == nil || !strings.Contains(err.Error(), "field is required") {
		t.Fatalf("expected field required error, got err=%v stdout=%s", err, stdout.String())
	}
}

func TestUnitCheckAppendixCoveragePassesWhenCompleteCLI(t *testing.T) {
	repoRoot := createCLISnapshotRepo(t)
	writeCLITestFile(t, filepath.Join(repoRoot, "docs/specs/units/stable/appendix/s_unit_demo_prompt.md"),
		"---\nunit: demo\nlayer: stable\n---\n\n# Stable Prompt\n")
	writeCLITestFile(t, filepath.Join(repoRoot, "docs/specs/units/candidate/appendix/c_unit_demo_prompt.md"),
		"---\nunit: demo\nlayer: candidate\n---\n\n# Candidate Prompt\n")
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	err := runUnit([]string{
		"check-appendix-coverage",
		"--object", "demo",
		"--repo-root", repoRoot,
	}, &stdout, &stderr)
	if err != nil {
		t.Fatalf("check-appendix-coverage failed: %v\nstdout=%s\nstderr=%s", err, stdout.String(), stderr.String())
	}
	if !strings.Contains(stdout.String(), "Appendix coverage: every stable appendix has a candidate counterpart") {
		t.Fatalf("expected valid coverage, got %s", stdout.String())
	}
}

func TestUnitCheckAppendixCoverageRejectsMissingCandidateCLI(t *testing.T) {
	repoRoot := createCLISnapshotRepo(t)
	writeCLITestFile(t, filepath.Join(repoRoot, "docs/specs/units/stable/appendix/s_unit_demo_prompt.md"),
		"---\nunit: demo\nlayer: stable\n---\n\n# Stable Prompt\n")
	var stdout bytes.Buffer
	...
	if err == nil || !strings.Contains(stdout.String(), "missing candidate appendix") {
		t.Fatalf("expected missing candidate appendix, got err=%v stdout=%s", err, stdout.String())
	}
}
